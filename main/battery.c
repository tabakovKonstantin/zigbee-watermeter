...
#define BATTERY_MAX_REASONABLE_MV (CONFIG_WATERMETER_BATTERY_FULL_MV + 100U)
...
{
    if (!s_battery.adc_handle || !s_battery.calibration_handle) {
    ...
    uint32_t raw_sum = 0;
    int raw = 0;
    for (uint32_t i = 0; i < (BATTERY_ADC_DISCARD_SAMPLES + CONFIG_WATERMETER_BATTERY_ADC_SAMPLES); i++) {
    ...
    uint32_t raw_avg = raw_sum / CONFIG_WATERMETER_BATTERY_ADC_SAMPLES;
    int adc_mv = 0;
    esp_err_t err = adc_cali_raw_to_voltage(s_battery.calibration_handle, raw_avg, &adc_mv);
    if (err != ESP_OK || adc_mv < 0) {
        ESP_LOGW(TAG, "Battery ADC calibration failed: %s", esp_err_to_name(err));
        return 0;
    }

    uint32_t battery_mv = battery_mv_from_gpio_mv((uint32_t)adc_mv,
                                                  CONFIG_WATERMETER_BATTERY_DIVIDER_TOP_OHM,
                                                  CONFIG_WATERMETER_BATTERY_DIVIDER_BOTTOM_OHM);
...
esp_err_t battery_init(void)
{
    ...
    ESP_RETURN_ON_ERROR(adc_oneshot_config_channel(s_battery.adc_handle, BATTERY_ADC_CHANNEL, &chan_config),
                        TAG,
                        "adc channel");
    ...
    return adc_cali_create_scheme_curve_fitting(&calibration_config, &s_battery.calibration_handle);
}
